...
import { PictoMapper } from "@/infrastructure/picto/mapper/picto.mapper";
...
export const getByWord = async (word: string): Promise<Pictograma[]> => {
  try {
    const { data } = await arasaacApi.get(
      `/es/search/${encodeURIComponent(word)}`,
    );
